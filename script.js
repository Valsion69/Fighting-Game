...
class Player {
  ...
  strike (player, enemy, attackDmg) {
    ...
    let attackAmt = Math.ceil(Math.random()*attackDmg)
    enemy.health -= attackAmt
    updateGame(p1,p2, gameState)
    return `${player.name} attacks ${enemy.name} for ${attackAmt}` 
  }  

  heal(player){
    let hp = Math.ceil(Math.random() *5)
    player.health += hp
    updateGame(p1,p2,gameState)
    return `${player.name} has heal for about ${hp} hp`
  }
